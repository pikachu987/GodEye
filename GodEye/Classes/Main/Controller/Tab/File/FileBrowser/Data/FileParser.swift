...
class FileParser {
    ...
    var excludesFileExtensions: [String]? {
        get {
            _excludesFileExtensions.map({$0.lowercased()})
        }
        set {
    ...
    var documentsURL: URL {
        fileManager.urls(for: .documentDirectory, in: .userDomainMask)[0] as URL
    }
    ...
    func filesForDirectory(_ directoryPath: URL) -> [FBFile]  {
        ...
        // Get contents
        do  {
            filePaths = try fileManager.contentsOfDirectory(at: directoryPath, includingPropertiesForKeys: [], options: [.skipsHiddenFiles])
        } catch {
            return files
        }
        // Parse
        ...
        // Sort
        files = files.sorted(by: { $0.displayName < $1.displayName })
        return files
    }
}
